...
for i in "${!COMMANDS[@]}"; do
    ...
    echo -e "Check for existence of $STATUS_FILE"

    if [[ -f "$STATUS_FILE" ]]; then
        echo "Skipping $RUN_NAME — status file already exists: $STATUS_FILE"
        continue   # go straight to next iteration
    fi
    echo -e "Status file does not already exist for run: $RUN_NAME - checking whether the job for this run can be submitted to the job group now"
	...
	JOBS_SUBMITTED=$(bjobs -g /mgriffit/perc 2>/dev/null | grep -v JOBID | wc -l)
