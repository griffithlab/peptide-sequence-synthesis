...
#isub -p false -i 'griffithlab/pvactools:5.5.2' -m 4
...
ALG_SUPPORTED_LENGTHS_FILE=$WORKING_BASE/peptide-sequence-synthesis/scripts/pvacbind_commands/algorithm_allele_length_key/supported_lengths_by_algorithm.txt
GLOBAL_MIN_LENGTH=8
...
for ALGO in "${ALGORITHMS[@]}"; do
  echo $ALGO
  echo "pvacbind valid_alleles -s human -p $ALGO > $WORKING_BASE/peptide-sequence-synthesis/scripts/pvacbind_commands/algorithm_allele_length_key/algorithm_alleles/${ALGO}.valid_alleles.txt"
  pvacbind valid_alleles -s human -p $ALGO > $WORKING_BASE/peptide-sequence-synthesis/scripts/pvacbind_commands/algorithm_allele_length_key/algorithm_alleles/${ALGO}.valid_alleles.txt
done
...
while IFS=":" read -r alg range; do
    ...
    for ((len=min; len<=max; len++)); do
        ...
        # Get the valid HLA alleles for this algorithm and create: ... do something with $alg and $len
